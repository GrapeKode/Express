...
export default class HelperMethods extends React.Component {
  ...
  deleteImage = (id) => {
    return this.fetch(
      this.state.apiURI.getProfileImage + id,
      {
        method: "DELETE",
        ...this.state.defaultOptions,
      },
  ...
  postImage = (image) => {
    return this.fetch(
      this.state.apiURI.getProfileImage,
      {
        method: "POST",
        ...this.state.defaultOptions,
        body: image,
  ...
  fetch = async (url, options, noHeaders = false) => {
    const headers = {}
    if (!noHeaders) headers["Content-Type"] = "application/x-www-form-urlencoded"
    if (this.isLoggedIn) {
