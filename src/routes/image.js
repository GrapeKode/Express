...
router.get("/image/:id", passport.authenticate("jwt", { session: false }), (req, res, next) => {
  if (req.params.id.length !== 24) return next({ status: 404, message: `Id-ul ${req.params.id} este incorect` })

  if (req.params.id) {
    gfs.files.find({}).toArray((err, files) => {
      if (err) return next(err)
      if (!files || files.length === 0) return next({ status: 404, message: `Imaginea cu id-ul ${req.params.id} n-a fost gasita` })
      for (let file of files) {
...
router.get("/image/full/:filename", (req, res, next) => {
  if (req.params.filename) {
    gfs.files.find({}).toArray((err, files) => {
      if (err) return next(err)
      if (!files || files.length === 0) return next({ status: 404, message: `Imaginea n-a fost gasita` })
      for (let file of files) {
...
router.post("/image", passport.authenticate("jwt", { session: false }), upload.single("profile"), (req, res, next) => {
  if (req.file === undefined) return next({ message: "No file has been received" })

  User.findOneAndUpdate({ _id: req.user._id }, { imageID: req.file.id }, { returnNewDocument: true }).exec((err, doc) => {
    if (err) return next(err)
    if (!doc) return next({ message: `Nu s-a putut face update utilizatorului cu id-ul ${req.params.id}` })
